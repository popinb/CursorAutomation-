...
# MAGIC - ? **Databricks LLM = Claude Sonnet 4.5**
...
# MAGIC 
# MAGIC **Options:**
# MAGIC - **databricks-llm** - Uses Claude Sonnet 4.5 (via Databricks Foundation Models)
...
if JUDGE_MODEL == "databricks-llm":
    ...
    # Try to get Anthropic API key from secrets
    ANTHROPIC_KEY = None
    key_source = None
    
    # Method 1: popin-secure-scope
    try:
        ANTHROPIC_KEY = dbutils.secrets.get("popin-secure-scope", "anthropic_key")
        key_source = "popin-secure-scope (shared)"
        print(f"   ? Retrieved Anthropic key from: {key_source}")
    except Exception as e:
        print(f"   ?? Cannot access popin-secure-scope/anthropic_key: {str(e)[:50]}...")
    
    # Method 2: Try openai_key scope (might have both keys)
    if not ANTHROPIC_KEY:
        try:
            ANTHROPIC_KEY = dbutils.secrets.get("popin-secure-scope", "anthropic_api_key")
            key_source = "popin-secure-scope/anthropic_api_key"
            print(f"   ? Retrieved Anthropic key from: {key_source}")
        except:
            pass
    ...
    if not ANTHROPIC_KEY:
        try:
            ...
            print("   ?? No personal Anthropic key found")
    ...
    if ANTHROPIC_KEY:
        ...
        print("\n   ?? To use Databricks LLM (Claude Sonnet 4.5):")
        print("      Ensure Anthropic API key is in Databricks secrets:")
...
if client:
    ...
    if client_type == "anthropic":
        print(f"   Model: Claude Sonnet 4.5 (Databricks LLM)")
...
# MAGIC %md
# MAGIC ## Cell 5: Create LLM Judge Evaluator (Supports Both OpenAI and Claude)
...
class LLMJudgeEvaluator:
    """LLM Judge Evaluator supporting both OpenAI and Anthropic Claude."""
    ...
    def __init__(self, client, model, metrics, ground_truth_data, client_type):
        ...
        self.client_type = client_type  # "openai" or "anthropic"
    ...
    def _call_llm(self, prompt):
        """Call LLM judge (supports both OpenAI and Anthropic)."""
        try:
            if self.client_type == "anthropic":
                ...
                response = self.client.messages.create(
                    ...
                    max_tokens=500,
                    temperature=0.1,
                    system="You are an expert evaluator. Provide responses in JSON format only.",
                    messages=[
    ...
    def evaluate_dataset(self, eval_data):
        ...
        provider_name = "Claude Sonnet 4.5" if self.client_type == "anthropic" else self.model
...
print("   ? Supports Claude Sonnet 4.5 (via Anthropic)")
...
if client is None:
    ...
    if client_type == "anthropic":
        print(f"   ? Evaluator ready with Claude Sonnet 4.5")
        print(f"   ? Using Databricks LLM (Anthropic)")
    else:
    ...
    if client_type == "anthropic":
        print(f"? Using Claude Sonnet 4.5 (Databricks LLM)")
    else:
...
# MAGIC 
# MAGIC ### ?? Model Options Tested:
# MAGIC - **databricks-llm** ? Claude Sonnet 4.5 (via Anthropic API)
...
# MAGIC - ? **databricks-llm uses Claude Sonnet 4.5**
...
# MAGIC - ? Automatic API key fallback (3 scopes tried)
