...
# MAGIC 3. Run Cell 3 (Configure LLM - select model)
...
# MAGIC 
# MAGIC **Select your judge model and configure API access**
...
if JUDGE_MODEL == "databricks-llm":
    ...
    print("   Note: This option uses Databricks Foundation Models")
    print("   (Implementation would use Databricks API)")
    client = None
    print("\n??  For this demo, please select an OpenAI model instead")
    
else:
    print(f"\n?? Initializing OpenAI connection via Zillow Labs proxy...")
    ...
    # Method 1: popin-secure-scope (primary - shared key)
    try:
        OPENAI_KEY = dbutils.secrets.get("popin-secure-scope", "openai_key")
        key_source = "popin-secure-scope (shared)"
        print(f"   ? Retrieved API key from: {key_source}")
    except Exception as e:
        print(f"   ?? Cannot access popin-secure-scope: {str(e)[:60]}...")
    ...
    # Initialize OpenAI client with Zillow Labs proxy
    if OPENAI_KEY:
        try:
            # Set environment variable
            os.environ["OPENAI_API_KEY"] = OPENAI_KEY
            
            # Initialize client with Zillow Labs base URL
            client = OpenAI(
            ...
            print(f"   ? Model {JUDGE_MODEL} is ready")
        ...
        print("\n? No OpenAI API key found in any scope")
        print("\n?? To use OpenAI models, you need API key access:")
        print("   Option 1: Ask notebook owner for access to 'popin-secure-scope'")
        print("   Option 2: Create your own scope:")
        print("             Scope name: user_{username}_secrets")
        print("             Key name: openai_key")
        print("   Option 3: Use 'databricks-llm' option instead")
        client = None
...
if client:
    ...
    print(f"   Using: {JUDGE_MODEL}")
    print(f"   Via: Zillow Labs OpenAI Proxy")
    print(f"   Key from: {key_source}")
else:
    print("? CANNOT PROCEED")
    print("   Please configure API access or select different model")
...
class LLMJudgeEvaluator:
    """LLM Judge Evaluator with Zillow Labs OpenAI proxy support."""
...
if client is None:
    ...
    print("\n?? To modify data: Edit JSON sections in Cell 2 and re-run")
    print("="*80)
...
# MAGIC - 8 Q&A samples about Cinderella
# MAGIC - 3 metrics: Story_Accuracy (binary), Response_Completeness (1-5), Child_Friendliness (%)
# MAGIC - Used Zillow Labs OpenAI proxy
# MAGIC 
# MAGIC **To modify:**
# MAGIC - Go back to Cell 2
# MAGIC - Edit the JSON data sections
# MAGIC - Re-run from Cell 2 onwards
# MAGIC 
# MAGIC **Features:**
# MAGIC - ? No file uploads needed
# MAGIC - ? All data hardcoded in JSON
