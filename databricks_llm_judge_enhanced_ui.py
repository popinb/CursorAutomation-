...
# MAGIC %md
# MAGIC # 🤖 LLM Judge Evaluation System - Enhanced UI with Multi-File Support
# MAGIC 
...
# MAGIC - ✅ **🆕 Multiple ground truth files per metric** (comma/semicolon separated)
# MAGIC - ✅ **🆕 Enhanced UI with multiple file upload options**
# MAGIC - ✅ **🆕 Auto-discovery of files in workspace**
...
# MAGIC 2. **Run Cell 2**: Configure file uploads (multiple options)
...
# MAGIC %md
# MAGIC ## Cell 2: Enhanced File Upload Configuration
# MAGIC **Multiple options for file uploads and discovery**
# MAGIC 
# MAGIC ### 🆕 **Enhanced File Upload Options:**
# MAGIC 
# MAGIC **Option 1: Single File Widgets** (Current approach)
# MAGIC - Specify individual files using widgets
# MAGIC - Good for simple setups
# MAGIC 
# MAGIC **Option 2: Multi-File Widgets** (New!)
# MAGIC - Specify multiple files separated by commas/semicolons
# MAGIC - Perfect for multiple ground truth files
# MAGIC 
# MAGIC **Option 3: Auto-Discovery** (New!)
# MAGIC - System automatically finds files in workspace
# MAGIC - No manual file specification needed
# MAGIC 
# MAGIC **Option 4: Pattern Matching** (New!)
# MAGIC - Use glob patterns to find files
# MAGIC - Great for batch processing
...
# Create enhanced file upload widgets
dbutils.widgets.dropdown(
    "upload_mode",
    "single_files",
    ["single_files", "multi_files", "auto_discovery", "pattern_matching"],
    "📁 File Upload Mode"
)
...
dbutils.widgets.text(
    ...
    "evaluation_data.csv", 
    "📊 Evaluation Data (CSV filename)"
)

dbutils.widgets.text(
    "metrics_config_path",
    "sample_metrics_config_multi_gt.csv",
    "📋 Metrics Configuration File (CSV filename)"
)

# Multi-file widgets (only shown when multi_files mode is selected)
dbutils.widgets.text(
    "ground_truth_files",
    "correct_answer.csv,accuracy_expert1.csv,accuracy_expert2.csv,helpful_answer.csv,helpfulness_rating1.csv,helpfulness_rating2.csv,safe_response.csv,complete_answer.csv",
    "📚 Ground Truth Files (comma separated)"
)

# Auto-discovery widgets
dbutils.widgets.text(
    "workspace_path",
    "/Workspace/Users",
...
dbutils.widgets.text(
    "file_patterns",
    "*.csv",
    "🎯 File Patterns (comma separated, e.g., *ground_truth*.csv, *evaluation*.csv)"
)

# Pattern matching widgets
dbutils.widgets.text(
    "glob_patterns",
    "**/evaluation_data.csv,**/sample_metrics_config*.csv,**/*ground_truth*.csv,**/*expert*.csv",
    "🔍 Glob Patterns (comma separated)"
)

# Get settings
UPLOAD_MODE = dbutils.widgets.get("upload_mode")
EVAL_DATA_PATH = dbutils.widgets.get("evaluation_data_path")
METRICS_CONFIG_PATH = dbutils.widgets.get("metrics_config_path")
GROUND_TRUTH_FILES = dbutils.widgets.get("ground_truth_files")
WORKSPACE_PATH = dbutils.widgets.get("workspace_path")
FILE_PATTERNS = dbutils.widgets.get("file_patterns")
GLOB_PATTERNS = dbutils.widgets.get("glob_patterns")

print("📁 ENHANCED FILE UPLOAD CONFIGURATION")
print("="*70)
...
def discover_files_in_workspace(search_path, patterns):
    """Discover files in workspace using patterns."""
    print(f"🔍 Discovering files in: {search_path}")
        ...
        for pattern in pattern_list:
            ...
            if files:
                ...
                for file in files:
                    ...
                    print(f"     - {filename}")
                    discovered_files[filename] = file
            else:
...
def discover_files_with_glob(glob_patterns):
    """Discover files using glob patterns."""
    print(f"🔍 Discovering files with glob patterns: {glob_patterns}")
    ...
    try:
        ...
        print(f"❌ Error discovering files with glob: {e}")
        return {}
...
def parse_ground_truth_files(gt_files_string):
    """Parse comma or semicolon separated ground truth filenames."""
    ...
    # If no separator found, treat as single file
    if not files:
        files = [gt_files_string.strip()]
    
    return files

def load_ground_truth_for_metric(metric_config, eval_df, discovered_files=None):
    """Load ground truth data for a specific metric, supporting multiple files."""
    ground_truth_files_string = metric_config.get('ground_truth_file_path', '')
    ground_truth_column = metric_config.get('ground_truth_column', 'ground_truth')
    
    # Parse multiple ground truth files
    ground_truth_files = parse_ground_truth_files(ground_truth_files_string)
    
    # If no files specified, try using ground_truth_column as filename
    if not ground_truth_files:
        if ground_truth_column and ground_truth_column != 'ground_truth':
            ground_truth_files = [f"{ground_truth_column}.csv"]
            print(f"   Using ground_truth_column as filename: {ground_truth_files[0]}")
        else:
            print(f"   No ground truth files specified for metric: {metric_config.get('name', 'unknown')}")
            return eval_df
    
    print(f"   Loading {len(ground_truth_files)} ground truth file(s) for metric: {metric_config.get('name', 'unknown')}")
    print(f"   Files: {ground_truth_files}")
    
    try:
        combined_gt_data = {}
        all_loaded_files = []
        
        # Load each ground truth file
        for gt_file in ground_truth_files:
            print(f"   Loading: {gt_file}")
            
            # Try discovered files first if available
            if discovered_files and gt_file in discovered_files:
                actual_path = discovered_files[gt_file]
                print(f"     Using discovered path: {actual_path}")
                gt_df = load_any_csv(actual_path, f"ground truth for {metric_config.get('name', 'unknown')}")
            else:
                gt_df = load_any_csv(gt_file, f"ground truth for {metric_config.get('name', 'unknown')}")
            
            if gt_df is None:
                print(f"   ⚠️ Failed to load: {gt_file}")
                continue
            
            all_loaded_files.append(gt_file)
            
            # Find common columns for merging
            common_cols = set(eval_df.columns) & set(gt_df.columns)
            if not common_cols:
                print(f"   ⚠️ No common columns found between eval data and {gt_file}")
            ...
            if ground_truth_column in gt_df.columns:
                ...
                if text_cols:
                    ...
                    print(f"   ⚠️ No suitable ground truth data column found in {gt_file}")
                    continue
            
            # Store the ground truth data for this file
            for _, row in gt_df.iterrows():
                sample_id = row[merge_col]
                gt_value = row[gt_data_col]
                
                if sample_id not in combined_gt_data:
                    combined_gt_data[sample_id] = []
                
                combined_gt_data[sample_id].append({
                    'file': gt_file,
                    'column': gt_data_col,
                    'value': gt_value
                })
        
        if not combined_gt_data:
            print(f"   ❌ No ground truth data loaded for metric: {metric_config.get('name', 'unknown')}")
            return eval_df
        
        # Combine ground truth data intelligently
        print(f"   ✅ Loaded ground truth from {len(all_loaded_files)} files")
        
        # Create combined ground truth column
        combined_gt_values = []
        for _, row in eval_df.iterrows():
            sample_id = row.get('sample_id', row.index)
            
            if sample_id in combined_gt_data:
                gt_entries = combined_gt_data[sample_id]
                
                # Combine multiple ground truth values
                if len(gt_entries) == 1:
                    # Single source
                    combined_value = gt_entries[0]['value']
                else:
                    # Multiple sources - combine intelligently
                    values = [entry['value'] for entry in gt_entries]
                    
                    # Strategy 1: Join with " | " separator
                    combined_value = " | ".join(values)
                    
                    # Strategy 2: Could also use other combination strategies
                    # - Take the longest/most detailed response
                    # - Take the first non-empty response
                    # - Use voting/consensus mechanisms
                    
                combined_gt_values.append(combined_value)
                print(f"   Sample {sample_id}: Combined {len(gt_entries)} ground truth sources")
            else:
                combined_gt_values.append('')
        
        # Add combined ground truth to the dataframe
        eval_df[ground_truth_column] = combined_gt_values
        
        coverage = sum(1 for v in combined_gt_values if v) / len(combined_gt_values) * 100
        print(f"   ✅ Ground truth coverage: {coverage:.1f}% ({sum(1 for v in combined_gt_values if v)}/{len(combined_gt_values)} samples)")
...
def load_metrics_config(file_path, discovered_files=None):
    """Load metrics configuration from CSV file with multi ground truth file support."""
    if not file_path or not file_path.strip():
        return None
    
    try:
        # Try discovered files first if available
        if discovered_files and file_path in discovered_files:
            actual_path = discovered_files[file_path]
            print(f"Using discovered metrics config: {actual_path}")
            df = load_any_csv(actual_path, "metrics config")
        else:
        ...
        # Show multi ground truth info
        for idx, row in df.iterrows():
            metric_name = row.get('name', f'metric_{idx}')
            gt_files_str = row.get('ground_truth_file_path', '')
            if gt_files_str:
                gt_files = parse_ground_truth_files(gt_files_str)
                print(f"   {metric_name}: {len(gt_files)} ground truth file(s) - {gt_files}")
...
# Discover files based on upload mode
discovered_files = {}

if UPLOAD_MODE == "auto_discovery":
    print("\n🔍 AUTO-DISCOVERY MODE")
    discovered_files = discover_files_in_workspace(WORKSPACE_PATH, FILE_PATTERNS)
    print(f"✅ Discovered {len(discovered_files)} files")
    
elif UPLOAD_MODE == "pattern_matching":
    print("\n🔍 PATTERN MATCHING MODE")
    discovered_files = discover_files_with_glob(GLOB_PATTERNS)
    print(f"✅ Discovered {len(discovered_files)} files")

# Load evaluation data
print(f"\n📊 Loading evaluation data using {UPLOAD_MODE} mode...")

if UPLOAD_MODE == "multi_files":
    # Use the multi-file approach
    eval_df_raw = load_any_csv(EVAL_DATA_PATH, "evaluation data")
else:
    # Use discovered files if available
    if discovered_files and EVAL_DATA_PATH in discovered_files:
        actual_path = discovered_files[EVAL_DATA_PATH]
        print(f"Using discovered evaluation data: {actual_path}")
        eval_df_raw = load_any_csv(actual_path, "evaluation data")
    else:
        eval_df_raw = load_any_csv(EVAL_DATA_PATH, "evaluation data")
...
metrics_config_df = load_metrics_config(METRICS_CONFIG_PATH, discovered_files)
...
if metrics_config_df is not None and len(metrics_config_df) > 0:
    ...
    for idx, metric_row in metrics_config_df.iterrows():
        ...
        ground_truth_files_string = metric_row.get('ground_truth_file_path', '')
        ground_truth_column = metric_row.get('ground_truth_column', 'ground_truth')
        
        if ground_truth_files_string and ground_truth_files_string.strip():
            print(f"\n📊 Processing ground truth for metric: {metric_name}")
            print(f"   Files: {ground_truth_files_string}")
            print(f"   Column: {ground_truth_column}")
            
            # Load ground truth for this specific metric
            eval_df = load_ground_truth_for_metric(metric_row, eval_df, discovered_files)
        else:
            print(f"   No ground truth files specified for metric: {metric_name}")
...
# MAGIC - `ground_truth_column` (optional): **Ground truth column name** (e.g., "correct_answer")
# MAGIC - `ground_truth_file_path` (optional): **Ground truth filenames** (comma or semicolon separated)
# MAGIC 
# MAGIC ### 🆕 Multi Ground Truth Features:
# MAGIC - **Multiple files per metric**: Each metric can have multiple ground truth files
# MAGIC - **Comma/semicolon separated**: Use `file1.csv,file2.csv` or `file1.csv;file2.csv`
# MAGIC - **Intelligent combination**: Multiple sources are combined with " | " separator
...
# MAGIC 
# MAGIC ### 📁 Example Metrics CSV with Multi Ground Truth:
# MAGIC ```csv
# MAGIC name,type,description,evaluation_prompt,threshold,ground_truth_column,ground_truth_file_path
# MAGIC accuracy_check,binary,Checks accuracy,"Evaluate...",1.0,correct_answer,"correct_answer.csv,accuracy_expert1.csv,accuracy_expert2.csv"
# MAGIC helpfulness,scale_1_5,Rate helpfulness,"Rate...",3.0,helpful_answer,"helpful_answer.csv;helpfulness_rating1.csv;helpfulness_rating2.csv"
# MAGIC ```
# MAGIC 
# MAGIC **Note**: The system will automatically load and combine all specified ground truth files!
...
if not METRICS_CONFIG_PATH or not METRICS_CONFIG_PATH.strip():
    ...
    print("\n📊 CSV Format Required:")
    print("   Columns: name, type, description, evaluation_prompt, threshold, ground_truth_column, ground_truth_file_path")
    print("   Types: binary, scale_1_5, percentage")
    print("   Multi GT: Use comma or semicolon separated filenames")
    print("   Example: accuracy_check,binary,Checks accuracy,\"Evaluate...\",1.0,correct_answer,\"file1.csv,file2.csv\"")
    ...
    # Display metrics summary with multi GT info
    for idx, row in METRICS_CONFIG_DATA.iterrows():
        ...
        gt_files_str = row.get('ground_truth_file_path', 'None')
        
        # Parse and count ground truth files
        gt_files = parse_ground_truth_files(gt_files_str)
        
        print(f"   {idx+1}. {metric_name} ({metric_type}) - threshold: {threshold}")
        print(f"      Ground Truth Column: {gt_column}")
        print(f"      Ground Truth Files: {len(gt_files)} file(s) - {gt_files}")
...
class LLMJudgeEvaluator:
    ...
    - Multi ground truth file support
    """
...
print("✅ Databricks LLM system loaded with enhanced UI and multi ground truth support")
...
# MAGIC - **🆕 Multi ground truth files per metric** for comprehensive evaluation
# MAGIC - **🆕 Enhanced UI with multiple file upload options**
# MAGIC 
# MAGIC ### 🆕 **Enhanced UI Features:**
# MAGIC - **Single File Mode**: Traditional widget-based file specification
# MAGIC - **Multi-File Mode**: Specify multiple files separated by commas/semicolons
# MAGIC - **Auto-Discovery Mode**: System automatically finds files in workspace
# MAGIC - **Pattern Matching Mode**: Use glob patterns to find files
# MAGIC - **Flexible file paths**: Support for various file locations and naming patterns
# MAGIC 
# MAGIC ### 🆕 **Multi Ground Truth Features:**
# MAGIC - **Multiple files per metric**: Each metric can have multiple ground truth files
# MAGIC - **Comma/semicolon separated**: Use `file1.csv,file2.csv` or `file1.csv;file2.csv`
...
# MAGIC 3. **Create multi ground truth files** for better evaluation coverage
...
# MAGIC - ✅ **🆕 Multi ground truth support** for comprehensive evaluation
# MAGIC - ✅ **🆕 Enhanced UI** for flexible file management
# MAGIC 
