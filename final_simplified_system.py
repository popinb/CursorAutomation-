...
# =============================================================================
# FINAL SIMPLIFIED FILE CONFIGURATION
# =============================================================================
...
# Ground truth file path widget - users can specify multiple files separated by semicolons
dbutils.widgets.text(
    "ground_truth_files",
    ...
    "📚 Ground Truth Files (full paths separated by semicolons)"
)
...
def create_ground_truth_mapping(gt_files_list):
    """Create mapping from filename to full path for all ground truth files."""
    mapping = {}
    missing_files = []
    ...
    for file_path in gt_files_list:
        if file_path and os.path.exists(file_path):
            ...
            print(f"✅ Mapped {filename} → {file_path}")
        else:
...
❌ MISSING GROUND TRUTH FILES:")
        for file_path in missing_files:
            print(f"   - {file_path}")
        print(f"
📋 Please upload the missing ground truth files to your Databricks workspace")
        print(f"   Then update the 'Ground Truth Files' widget with the correct paths")
    
    return mapping

# Parse ground truth files from UI
GROUND_TRUTH_FILES_LIST = parse_ground_truth_files(GROUND_TRUTH_FILES_STRING)
print(f"
📚 Parsed {len(GROUND_TRUTH_FILES_LIST)} ground truth files:")
for i, file_path in enumerate(GROUND_TRUTH_FILES_LIST, 1):
    print(f"   {i}. {file_path}")

# Create mapping from filename to full path
GROUND_TRUTH_PATH_MAPPING = create_ground_truth_mapping(GROUND_TRUTH_FILES_LIST)
...
def load_ground_truth_for_metric(metric_config, eval_df):
    """Load ground truth data for a specific metric - column agnostic."""
    ...
    try:
        ...
        # Find common columns for merging (usually sample_id)
        ...
        # Find the ground truth data column - COLUMN AGNOSTIC
        gt_data_col = None
        ...
        if ground_truth_column in gt_df.columns:
            ...
            # Look for any text column that could be ground truth data (COLUMN AGNOSTIC)
            ...
            if text_cols:
                ...
                print(f"   Using '{gt_data_col}' as ground truth data column (column agnostic)")
            else:
                print(f"   ❌ No suitable ground truth data column found in {ground_truth_file}")
                print(f"   📋 Please ensure the file has at least one text column with ground truth data")
                return eval_df
...
📝 Creating sample data for demonstration...")
    ...
    # Standardize column names - EVALUATION FILE HAS PREDICTABLE COLUMNS
