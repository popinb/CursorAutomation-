...
# Create action widget (only one always visible)
try:
    dbutils.widgets.dropdown("action", "add", ["add", "edit", "delete"], "Action")
except:
    pass

action = dbutils.widgets.get("action")

# Conditionally show/hide widgets based on action
if action == "add":
    try:
        ...
        dbutils.widgets.text("m_gt_file", "", "Ground Truth File (optional)")
        dbutils.widgets.text("m_gt_col", "", "GT Column (optional)")
    except:
        pass
    try:
    ...
    try:
        ...
        pass
    for widget_name in ["m_name", "m_type", "m_desc", "m_rubric", "m_threshold", "m_gt_file", "m_gt_col"]:
        try:
...
if action == "add" and dbutils.widgets.get("m_name").strip():
    ...
    # Clear form widgets
    for w in ["m_name", "m_desc", "m_rubric", "m_threshold", "m_gt_file", "m_gt_col"]:
        dbutils.widgets.remove(w)
        dbutils.widgets.text(w, "", w)
    
    print(f"\n? ADDED NEW METRIC: {new_metric['name']}")
    print("?? Set Action='view' and re-run to see updated table")
